        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($user) {
                ...
                if (getenv('APP_ENV') === 'local') {
                    $message = "MODE LOCAL ACTIF : <br> L'email n'a pas été réellement envoyé mais simulé (voir <code>storage/mails.log</code>). <br><br> <a href='$link' style='color:blue; text-decoration:underline;'>CLIQUEZ ICI POUR TESTER LE LIEN DE RÉINITIALISATION</a>";
                } else {
                    $message = "Si cette adresse est enregistrée associée à un compte actif, vous recevrez un email avec les instructions.";
                }
            } else {
                // Par mesure de sécurité, on affiche le même message même si l'utilisateur n'existe pas en prod
                if (getenv('APP_ENV') === 'local') {
                    $message = "MODE LOCAL ACTIF : Cet email n'existe pas ou n'est pas actif dans la base de données. (" . htmlspecialchars($email) . ")";
                } else {
                    $message = "Si cette adresse est enregistrée associée à un compte actif, vous recevrez un email avec les instructions.";
                }
            }
