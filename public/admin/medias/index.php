    ...
    <div class="admin-panel" style="max-width: 1200px; margin: 0 auto; padding: 20px;">
        ...
        <div class="media-grid">
            <?php foreach ($medias as $media): ?>
                ...
                $mediaId = $media['id'] ?? 0;
                $mediaSiteId = $media['site_id'] ?? 1;

                // Correction dynamique du chemin d'accès
                $displayPath = $filePath;
                // Si l'image vient historiquement des dossiers du site (et pas d'un recent upload)
                if (strpos($filePath, 'assets/') === 0) {
                    if ($mediaSiteId == 1) {
                        $displayPath = 'noeux/' . $filePath;
                    } elseif ($mediaSiteId == 2) {
                        $displayPath = 'reserve/' . $filePath;
                    }
                }
                ?>
